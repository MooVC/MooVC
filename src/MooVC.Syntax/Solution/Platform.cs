namespace MooVC.Syntax.Solution
{
          ...
          IValidatableObject
    {
        ...
        {
            return $"{nameof(Platform)} {{ {nameof(IsUndefined)} = {DebuggerDisplayFormatter.Format(IsUndefined)}, {nameof(Project)} = {DebuggerDisplayFormatter.Format(Project)}, {nameof(Solution)} = {DebuggerDisplayFormatter.Format(Solution)} }}";
        }
    }
}