...
public static partial class DiagnosticsRelayExtensions
{
    ...
    /// <param name="args">The arguments of the message (if any).</param>
    public static async void Trace(this IDiagnosticsRelay? diagnostics, string message, CancellationToken cancellationToken, params object[] args)
    {
        await diagnostics
            .TraceAsync(message, cancellationToken, args)
            .ConfigureAwait(false);
    }
    ...
    {
        await diagnostics
            .TraceAsync(cause, message, cancellationToken, args)
            .ConfigureAwait(false);
    }
}