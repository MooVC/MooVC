...
public static partial class DiagnosticsRelayExtensions
{
    ...
    /// <param name="args">The arguments of the message (if any).</param>
    public static async void Warning(this IDiagnosticsRelay? diagnostics, string message, CancellationToken cancellationToken, params object[] args)
    {
        await diagnostics
            .WarningAsync(message, cancellationToken, args)
            .ConfigureAwait(false);
    }
    ...
    {
        await diagnostics
            .WarningAsync(cause, message, cancellationToken, args)
            .ConfigureAwait(false);
    }
}