...
public sealed class WhenValidateIsCalled
{
    ...
    public async Task GivenMultiLineConditionThenValidationErrorReturned()
    {
        ...
        await Assert.That(results[0].MemberNames).Contains(nameof(TargetTask.Condition));
    }
    ...
    public async Task GivenUnnamedNameThenValidationErrorReturned()
    {
        ...
        await Assert.That(valid).IsFalse();
        _ = await results.Single();
        await Assert.That(results[0].MemberNames).Contains(nameof(TargetTask.Name));
    }
    ...
    public async Task GivenUndefinedOutputThenValidationErrorReturned()
    {
        ...
        await Assert.That(valid).IsFalse();
        _ = await results.Single();
        await Assert.That(results[0].MemberNames).Contains(nameof(TargetTask.Outputs));
    }
}