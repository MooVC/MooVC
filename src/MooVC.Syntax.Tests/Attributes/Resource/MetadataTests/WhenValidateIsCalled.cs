...
public sealed class WhenValidateIsCalled
{
    ...
    {
        // Arrange
        Metadata subject = MetadataTestsData.Create(xmlSpace: Snippet.From($"alpha{Environment.NewLine}beta"));
        var context = new ValidationContext(subject);
        var results = new List<ValidationResult>();
        ...
        valid.ShouldBeFalse();
        _ = results.ShouldHaveSingleItem();
        results[0].MemberNames.ShouldContain(nameof(Metadata.XmlSpace));
    }
}