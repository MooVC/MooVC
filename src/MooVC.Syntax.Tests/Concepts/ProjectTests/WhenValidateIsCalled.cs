...
public sealed class WhenValidateIsCalled
{
    ...
    public async Task GivenUndefinedImportThenValidationErrorReturned()
    {
        ...
        await Assert.That(results[0].MemberNames).Contains(nameof(Project.Imports));
    }
    ...
    public async Task GivenUnspecifiedSdkThenValidationErrorReturned()
    {
        ...
        await Assert.That(valid).IsFalse();
        _ = await results.Single();
        await Assert.That(results[0].MemberNames).Contains(nameof(Project.Sdks));
    }
    ...
    public async Task GivenUndefinedResourceThenValidationErrorReturned()
    {
        ...
        await Assert.That(valid).IsFalse();
        _ = await results.Single();
        await Assert.That(results[0].MemberNames).Contains(nameof(Project.Resources));
    }
}